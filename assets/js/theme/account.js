...
export default class Account extends PageManager {
    ...
    onReady() {
        ...
        const unfortArray = document.querySelectorAll('.account p');
        unfortArray.forEach((target) => {
            const elm = target;
            if (elm.innerHTML.indexOf('Unfortunately this product is no longer for sale so it cannot be reordered.') !== -1) {
                elm.style.display = 'none';
            } else {
                elm.style.classList.add('sub-text');
            }
        });
    }
