...
} from './common/utils/translations-utils';
...
export default class Cart extends PageManager {
    onReady() {
        ...
        this.$cartContent = $('#cart-preview-dropdown');
    ...
    refreshContent(remove) {
        const $cartItemsRows = $('[data-item-row]');
        ...
        utils.api.cart.getContent(options, (err, response) => {
            if (response) {
                ...
                const quantity = $('[data-cartQuantity]').attr('data-cartQuantity') || 0;
    ...
    bindCartEvents() {
        ...
        $('.cart-item-qty-input').on('focus', function onQtyFocus() {
            preVal = this.value;
        }).change(event => {
            const $target = $(event.currentTarget);
            event.preventDefault();
    ...
    bindEvents() {
        ...
        const shippingErrorMessages = {
            country: "The 'Country' field cannot be blank.",
            province: "The 'State/Province' field cannot be blank.",
        };
        this.shippingEstimator = new ShippingEstimator($('[data-shipping-estimator]'), shippingErrorMessages, this);
        $('.btn--shopping').on('click', () => {
            window.history.back();
        });
        $('.remove-coupon').on('click', (e) => {
            e.preventDefault();
            const $couponRemove = $(e.currentTarget);
            const link = $couponRemove.attr('href');
            this.$overlay.show();
 
            utils.api.getPage(link, {}, (err, content) => {
                this.refreshContent();
            })
        });
        $('.preview-cart-close, .cart-preview-background-el').on('click', () => {
